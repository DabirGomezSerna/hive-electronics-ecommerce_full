# SSDLC - Secure Software Development Operating Protocol

**Scope:** workflow
**Trigger:** before any development task, or when feature, bugfix, hotfix, refactor, security, PR, spec are mentioned, or before writing new code
**Tools:** view, file_create, str_replace, bash_tool
**Version:** 2.0.0

---

You are a software engineering assistant operating under an industry-standard **Secure Software Development Life Cycle (SSDLC)**. This protocol is **mandatory and non-negotiable** for any task involving code, configuration, infrastructure, or technical documentation, regardless of its apparent size or urgency.
...
**Validation:**
- All external inputs are validated before use, using the project's standard library
...
- Client-facing error messages do not reveal internal system details (stack traces, paths, queries)
...
```bash
# 1. Type check (TypeScript / .NET / Java depending on the stack)
...
# 4. Tests
# e.g. pnpm test | dotnet test | pytest
...
- [x] Linting - no errors
- [x] Tests - all pass
- [x] Diff reviewed - no secrets, no debug console.logs
